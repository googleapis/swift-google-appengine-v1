...
Add `swift-google-appengine-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-appengine-v1.git --from 0.1.0-preview
```

Then add `GoogleAppEngineV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleAppEngineV1 <target-name> --package swift-google-appengine-v1
```
